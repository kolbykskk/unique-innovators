...
RSpec.describe GigsController, type: :controller do
  ...
  describe "GET #create" do
    it "increases the number of gigs by 1" do
      expect{ post :create, params: { user_id: user.id, gig: { title: FFaker::Job.title[3..30], category: FFaker::Lorem.word, description: FFaker::Lorem.paragraph[50..500], delivery_time: '2 days', allow: "Online Only" } } }.to change(Gig,:count).by (1)
    end

    it "assigns the new gig to @gig" do
      post :create, params: { user_id: user.id, gig: { title: FFaker::Job.title[3..30], category: FFaker::Lorem.word, description: FFaker::Lorem.paragraph[50..500], delivery_time: '1 day', allow: "Online or Meetups", location: FFaker::Address.city } }
      expect(assigns(:gig)).to eq user.gigs.last
    end

    it "redirects to the newly created gig" do
      post :create, params: { user_id: user.id, gig: { title: FFaker::Job.title[3..30], category: FFaker::Lorem.word, description: FFaker::Lorem.paragraph[50..500], delivery_time: '20 days', location: FFaker::Address.city, allow: "Meetups Only" } }
      expect(response).to redirect_to Gig.last
    end

    it "fails if no location is set unless allow is set to online-only" do
      expect{ post :create, params: { user_id: user.id, gig: { title: FFaker::Job.title[3..30], category: FFaker::Lorem.word, description: FFaker::Lorem.paragraph[50..500], delivery_time: '2 days', allow: "Meetups Only" } } }.to change(Gig,:count).by (0)
    end

    it "sets the latitude and longitude when given a location" do
      post :create, params: { user_id: user.id, gig: { title: FFaker::Job.title[3..30], location: "Binghamton NY, USA", category: FFaker::Lorem.word, description: FFaker::Lorem.paragraph[50..500], delivery_time: '1 day', allow: "Online or Meetups" } }
  ...
  describe "GET #destroy" do
    ...
    it "redirects to the user dashboard" do
      get :destroy, params: { id: gig.id }
      expect(response).to redirect_to gig_path
    end
  end
